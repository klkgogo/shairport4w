...
protected:
	virtual void OnRequest(CRtpEndpoint* pClient, shared_ptr<CRtpPacket>& packet) = 0;

protected:
	inline void PutPacketToPool(shared_ptr<CRtpPacket>& p)
	{
		if (p != NULL)
		{
	...
	inline shared_ptr<CRtpPacket> GetNewPacketFromPool()
	{
		shared_ptr<CRtpPacket> pResult;
		...
		{
			m_mtxPacketPool.Unlock();
			pResult = make_shared<CRtpPacket>();
		}
		if (pResult != NULL)
			pResult->Init();
...
public:
	CRtpEndpoint(const char* strName, int nType = SOCK_DGRAM)
	{
		...
		m_strReceiveFromName= string("RtpRequest.From.") + string(strName);
		m_strSendToName		= string("RtpSend.To.") + string(strName);
	...
	{
		ATLASSERT(pRequestHandler != NULL);
		ATLASSERT(psaPeer != NULL);
	...
	void OnRequest(SOCKET sd)
	{
		...
		shared_ptr<CRtpPacket> p = m_pRequestHandler->GetNewPacketFromPool();
		...
		if (p != NULL)
		{
			...
			{
				p->m_nLen = sock.recv(p->m_Data, 16);

				int nDataLen = p->getSeqNo() - RTP_DATA_OFFSET;

				if ((nDataLen+RTP_DATA_OFFSET) > RAOP_PACKET_MAX_SIZE)
					p->m_Data.Reallocate(nDataLen+RTP_DATA_OFFSET);
				p->m_nLen = RTP_DATA_OFFSET + sock.recv(p->m_Data+RTP_DATA_OFFSET, nDataLen);

				p->setPayloadType(PAYLOAD_TYPE_STREAM_DATA);
				p->putSeqNo(m_nSeqGen++);
			}
#endif
			if (p->m_nLen > 0)
			{
	...
	static bool CreateEndpoint(const char* strName, IRtpRequestHandler* pRequestHandler, const PSOCKADDR_IN psaPeer, shared_ptr<CRtpEndpoint>& udp_endpoint, shared_ptr<CRtpEndpoint>* tcp_endpoint = NULL)
	{
		if (udp_endpoint != NULL)
			udp_endpoint.reset();

		udp_endpoint = make_shared<CRtpEndpoint>(strName);
		...
		do
		{
			...
			if (udp_endpoint->Run(pRequestHandler, psaPeer, htons(nPort)))
			{
				...
				{
					shared_ptr<CRtpEndpoint>& _tcp_endpoint = *tcp_endpoint;

					_tcp_endpoint = make_shared<CRtpEndpoint>(strName, SOCK_STREAM);
...
protected:
	...
	CMyMutex				m_mtx_sa;
	int						m_nType;
	string					m_strReceiveFromName;
	string					m_strSendToName;
public:
	USHORT					m_nSeqGen;
};