    ...
    )
{
	...
	if (pDNSServiceQueryRecord != NULL)
	{
    ...
    )
{
	...
	ATLASSERT(pEvent->m_pCallback != NULL);

	if (txtRecord != NULL && txtLen > 0)
		pEvent->m_strTXT		= ci_string((const char*)txtRecord, txtLen);
	else
		pEvent->m_strTXT.clear();

	ci_string strHostTarget = hosttarget;
...
bool CDnsSD_Register::Start(BYTE* addr, LPCSTR strApName, BOOL bNoMetaInfo, BOOL bPassword, USHORT nPort)
{
	char buf[512];
...
{
	if (serviceName != NULL && m_hWnd != NULL && ::IsWindow(m_hWnd))
	{
