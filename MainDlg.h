...
public:
...

	{
		...
		ATLVERIFY(m_strSoundRedirection.LoadString(IDS_NO_REDIRECTION));
		...
		m_bRedirStartEarly					= false;
		m_hRedirProcess						= NULL;
		m_hRedirData						= INVALID_HANDLE_VALUE;
		m_hAccelerator						= NULL;
		m_bSoundRedirection					= false;

		if (CHairTunes::GetSoundId() != NULL)
			m_strSoundcardId = CHairTunes::GetSoundId();
	}
	...
	inline bool HasMultimediaControl(shared_ptr<CDacpService>* ppResult = NULL)
	{
		bool bResult = false;
	...
	inline void SetDacpID(ci_string strDacpID, ci_string strActiveRemote)
	{
	...
	inline bool IsCurrentContext(shared_ptr<CRaopContext>& pRaopContext)
	{
		return m_pRaopContext == pRaopContext;
	}
...
public:
	HICON															m_hIconSmall;
	WTL::CString													m_strApName;
	WTL::CString													m_strPassword;
	WTL::CString													m_strSoundRedirection;
	...
	WTL::CString													m_strTimeInfo;
	WTL::CString													m_strTimeInfoCurrent;
...
protected:
	...
	shared_ptr<CRaopContext>										m_pRaopContext;
	CMyMutex														m_mtxDacpService;
	ci_string														m_strActiveRemote;
	ULONGLONG														m_DACP_ID;
	map< ULONGLONG, shared_ptr<CDacpService> >						m_mapDacpService;
	...
	WTL::CString													m_strReady;

	void PutMMState(typeMMState nMmState);
	void TerminateRedirection();
...
#ifdef _WITH_PROXY
		MESSAGE_HANDLER(WM_RAOP_REG, OnRaopRegistered)
#endif
...
#ifdef _WITH_PROXY
	LRESULT OnRaopRegistered(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/);
#endif
